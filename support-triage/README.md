...
- **No SLA escalation clock.** Nothing currently tracks whether an urgent ticket has gone unaddressed for too long. Left out to keep this system focused on getting classification, routing, and error handling right before expanding scope.
...
**What scaling this up actually looks like.** The parts of this system that are simplified for its current scope, a flat duplicate window, no SLA clock, single intake channel, are exactly the parts called out above as deliberate scope decisions, not structural limits. Multi-channel intake means adding another trigger that feeds into the same dedup and classification logic. An SLA clock means a scheduled check against the audit log's timestamps. None of it requires re-architecting what's already built; it's addition, not replacement, which is the actual test of whether a system was built to grow or built to be thrown away.

**What changes if this runs at enterprise volume instead of a small team's inbox.** This build was scoped for a small team's ticket volume, but the underlying decisions were made with a larger version in mind, not against it. A few concrete examples of what actually changes:

- **Storage.** Google Sheets is fine at hundreds or low thousands of tickets a month. Past that, read/write latency and the flat-file structure become the bottleneck, not the automation logic. The audit log module is the only piece that needs to change, swap it for Postgres, Supabase, or BigQuery, and every module upstream of it is untouched, since none of them know or care where the log lands.
- **Multi-tenant isolation.** A single client's version of this pipeline needs no tenant awareness at all. A version serving multiple clients or business units through one shared scenario would add a tenant identifier at intake and carry it through every downstream record, classification, log entry, and Slack routing decision, so data from one client is never visible to or mixed with another's. That's a mapping addition to existing modules, not a redesign of the branching logic itself.
- **Throughput.** Make and n8n both support queued/batched processing and rate-limit-aware retry logic for exactly this reason. At meaningfully higher ticket volume, the two Gemini calls per ticket would move to a batched or asynchronous pattern rather than a synchronous call-and-wait per ticket, the same architectural shift any high-volume API integration needs regardless of what the API is being used for.
- **Access and audit requirements.** A larger client or a regulated industry adds requirements this build doesn't currently have, role-based access to the audit log, retention policy, and often a requirement that AI-generated content (the suggested replies) is clearly flagged as such wherever it's stored, not just in the Slack draft. None of that changes the classification or routing logic, it's a layer added around the existing pipeline.

The point isn't that this system is enterprise-scale today, it's a five-stage system built and tested at small-team volume. The point is that the architecture, dedup before AI calls, independent error handling per external call, a durable log decoupled from the notification layer, is the same shape a larger system needs, just with heavier infrastructure under each piece. Nothing here was built in a way that would need to be thrown out to grow.
